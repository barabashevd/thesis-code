...
def load_bluehex_data(task, test_begin_time, test_end_time, families_cnt, is_normalize,
    ...
    if os.path.exists(saved_data_path):
        ...
        if task == 'binary':
            ...
            meta_df = load_meta('bodmas', general_data_folder)
            ...
            X, y = load_npz_data('bodmas', general_data_folder)
            ...
            X, _ = load_npz_data('bodmas', general_data_folder)
            ...
            y_test_list = []
            X_test_list, y_test_raw_list = prepare_testing_set(test_begin_time, test_end_time, general_data_folder,
                                                               task='multiclass')
...
def prepare_testing_set(test_begin_time, test_end_time, general_data_folder, task='binary'):
    ...
    if task == 'binary':
        ...
        X_test_list = [np.array(X[m[~m.family.isna()].index], dtype=np.float32) for m in
                       meta_test_list]  # do not include benign
