...
def evaluate_prediction_on_testing(model, phase, X_test, y_test, threshold, test_begin_time, test_end_time, seed,
    ...
    if detail:
        report_detailed_classification_result(y_test, y_test_pred, threshold, phase, test_begin_time, test_end_time,
                                              seed,
...
def report_detailed_classification_result(y_test, y_test_pred, threshold, phase, test_begin_time, test_end_time, seed,
    ...
    with open(misclassified_result_path, 'w') as f:
        ...
        meta_df = multiple_data.load_meta('bodmas', general_data_folder='multiple_data')
...
def plot_confusion_matrix(cm, top_family, mapping, save_fig_name, phase):
    ...
    if phase == 'val':
        ...
        ax.set_xticklabels([mapping[str(t)] for t in range(no_of_axes - 1)] + ['unseen'], fontsize=fontsize,
                           rotation='vertical')
        ax.set_yticklabels([mapping[str(t)] for t in range(no_of_axes - 1)] + ['unseen'], fontsize=fontsize,
                           rotation='horizontal')
