...
def parse_multiple_dataset_args():
    ...
    p.add_argument('--diversity', choices=['no', 'size', 'family', 'timestamp', 'timestamp_part', 'legacy', 'packed',
                                           'family_fixed_size'],
    ...
    p.add_argument('--setting-name',
                   help='name for this particular setting, for saving corresponding data, model, and results')
    ...
    p.add_argument('--w',
                   type=int,
                   default=2500,
                   help='Sliding window size for D3')

    p.add_argument('--rho',
                   type=float,
                   default=0.1,
                   help='Rho parameter for D3')

    p.add_argument('--auc',
                   type=float,
                   default=0.9,
                   help='AUC parameter for D3')
...
def parse_drift_args():
    ...
    p.add_argument('--sample-ratio', default=0.01, type=float,
                   help='how many samples to add back to the training set for retraining to combat concept drift.')
...
def get_ember_sample_idx_by_pred_proba(X_sample_full, y_sample_full,
    ...
    report_path = os.path.join(SUB_REPORT_FOLDER,
                               f'{args.classifier}_{test_begin_time}_ranked_proba_sample_{sample_month_str}.csv')
