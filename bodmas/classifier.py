    ...
    def train(self, X_train, y_train, task, families_cnt, retrain, params={}):
        if not os.path.exists(self.saved_model_path) or retrain:
            ...
            if task != 'binary':
                ...
                # Ensure binary classification parameters are set as needed
            ...
            dump(clf, self.saved_model_path)
            logging.info(f'Training finished, time: {timer() - begin:.1f} seconds')
        else:
            logging.info("Loading pre-trained LightGBM model...")
            clf = load(self.saved_model_path)

        self.model = clf  # Store the trained or loaded model
        return self
...
class GBDTClassifierOld(object):
    def __init__(self, saved_model_path):
        self.saved_model_path = saved_model_path
