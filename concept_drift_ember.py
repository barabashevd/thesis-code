...
def main():
    ...
    if ember_ratio < 1.0:
        ...
        X_ember = X_ember[select_idx]
        y_ember = y_ember[select_idx]
        '''NOTE: use 24% Ember (30% * 80% = 24%) to train Transcend due to its high computation cost'''
        X_train_old, X_val, y_train_old, y_val = train_test_split(X_ember, y_ember, test_size=0.2,
                                                                  random_state=2, shuffle=True)
        logging.info(
            f'Ember selected: {select_num}, X_train_old: {X_train_old.shape}, X_val: {X_val.shape}, y_train_old: {y_train_old.shape}, y_val: {y_val.shape}')

    '''use 80% Ember as the base for accumulative retraining'''
    X_train_base, X_val_base, y_train_base, y_val_base = train_test_split(X_ember, y_ember, test_size=0.2,
                                                                          random_state=2, shuffle=True)

    logging.info(
        f'before adding {setting} sampling, training label: {Counter(y_train_base)}, validation label: {Counter(y_val_base)}')
    ...
    # ---------------------------------------------------------------------------------- #
    # 4. accumulatively sampling from the BODMAS data and test on the next month   #
    # ---------------------------------------------------------------------------------- #
    ...
    for idx, sample_month, test_month in iteration:
        ...
        sum_samples += sample_num
        if 'random' in setting:
            seed(SEED)
            sample_idx = np.random.choice(range(X_sample_full.shape[0]), size=sample_num, replace=False)

            # record which samples were selected
            detect_path = os.path.join(REPORT_FOLDER, f'intermediate',
                                       f'gbdt_detect_results_{sample_month_str}_random_{SEED}.csv')
            ...
            y_sample_pred_label = np.array(y_sample_pred > fpr_threshold, dtype=np.int64)
        ...
        if clf != 'rf':
            ...
            for fpr_target_on_val in [0.01, 0.001]:
            ...
            evaluate.write_result_to_report(fpr_list_all, tpr_list_all, f1_list_all, auc_score_list, REPORT_PATH,
                                            is_rf=False)
        else:
    ...
    logging.info(f'total samples {sum_samples}')
