...
def main():
    ...
    log_path = './logs/concept_drift_md3'
    if args.quiet:
    ...
    folder_name = 'multiple_models/md3'  # path to saved models
    ...
    MODEL_POSTFIX = f'_ember_{first_month}_sample_{second_month}_ratio_{sample_ratio}_random_{SEED}'
    SAVED_DATA_PATH = os.path.join(DATA_FOLDER, f'X_and_y_{POSTFIX}.h5')

    # this code loads data for the first month, second (aka testing month) is ignored
    X_first_month, y_first_month, _, _, = multiple_data.load_ember_drift_data(X, y, first_month, second_month,
    ...
    SAVED_MODEL_PATH = os.path.join(folder_name, f'md3_base_initial_setup_{SEED}.txt')
    SAVED_MODEL_PATH = 'multiple_models/pretrain_ember/ember_gbdt_model_seed1.txt'
    ...
    base_model = gbdt_clf.train(X_train, y_train, task, families_cnt, False, config['gbdt_params'])
    ...
    md3_lgb = MD3(K=15, sensitivity=0.1, bound=0.25)
    md3_lgb.set_reference(X_train, y_train, task, families_cnt, retrain, folder_name)
    ...
    report_path = os.path.join(REPORT_FOLDER, f'md3_{sample_ratio}', f'md3_{task}_report_{POSTFIX}_{timer()}.csv')
    ...
    for idx, sample_month, test_month in iteration:
        ...
        if idx == 1:
            # first month is already included in training
            logging.info(f'Skipping first month')
            continue
        ...
        for i, (X_chunk, y_chunk) in enumerate(zip(X_sample_chunks, y_sample_chunks)):
            ...
            if drift_suspected:
                ...
                drift = md3_lgb.confirm_drift(X_chunk, y_chunk, base_model)
                ...
                if drift:
                    ...
                    # use 1 % of new data
                    X_new_skip, X_new_train, y_new_skip, y_new_train = train_test_split(X_new, y_new, test_size=0.01,
                                                                                        random_state=2, shuffle=True)

                    # use 80 % of base
                    X_all_base, X_all_test, y_all_base, y_all_test = train_test_split(X_all, y_all, test_size=0.2,
                                                                                      random_state=2, shuffle=True)

                    X_train = np.vstack((X_new_train, X_all_base))
                    y_train = np.hstack((y_new_train, y_all_base))
                    if X_new_skip.shape[0] > labelled_untrained:
                    ...
                    SAVED_MODEL_PATH = os.path.join(MODELS_FOLDER, f'md3-{postfix}.txt')
                    gbdt_clf = bodmas_classifier.GBDTClassifierOld(saved_model_path=SAVED_MODEL_PATH)
                    logging.debug(f'model path: {SAVED_MODEL_PATH}')
