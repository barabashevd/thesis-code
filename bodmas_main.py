...
def main():
    ...
    clf = args.bodams_classifier
    retrain = args.retrain

    if clf == 'gbdt':
        SAVED_MODEL_PATH = os.path.join(MODELS_FOLDER, f'gbdt{MODEL_POSTFIX}_r{SEED}.txt')
        gbdt_clf = classifier.GBDTClassifierOld(saved_model_path=SAVED_MODEL_PATH)
        ...
        SAVED_MODEL_PATH = os.path.join(MODELS_FOLDER,
                                        f'mlp_{dims_str}_lr{lr}_b{batch}_e{epochs}_d{dropout}{MODEL_POSTFIX}.h5')
...
def multiclass_prediction_helper(clf, model, X, y, families_cnt, top_n_acc, mapping, report_folder, phase):
    ...
    VAL_CM_FIG_PATH = os.path.join(FIG_SAVE_FOLDER,
                                   f'{clf}_family_{families_cnt}_topacc_{top_n_acc}_{phase}_confusion_matrix.png')
